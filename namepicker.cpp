...
NamePicker::NamePicker() : hstry() {
  for (const auto &nm : lst)
    if (hstry.check(nm))
      st.push_back(nm);
}
const std::string &NamePicker::pick() const {
  return lst[rdm(lst.size())];
}

std::string NamePicker::pick_no_recent() {
  if (st.empty())
...
int NamePicker::rdm(std::vector<std::string>::size_type sz) const {
